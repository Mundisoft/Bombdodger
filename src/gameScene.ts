...
export default class GameScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const playerOne: Phaser.Physics.Arcade.Sprite = this.players
      .create(0, 0, 'Owl')
      .setData({
        name: 'Player 1',
        key: 'Owl',
    ...
    this.players.children.iterate(function(
      ...
      this.players.children.iterate(function(
        ...
        if (!player.getData('dead')) {
          ...
          if (player.getData('up').isDown && player.body.blocked.down) {
            ...
            player.getData('up').isDown &&
            player.getData('doublejumps') > 1
          ) {
            player.setVelocityY(-this.jumpspeed);
